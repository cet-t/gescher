...
fn main() -> Result<()> {
    ...
    ui.on_setting_changed(move || {
        if let Some(ui) = ui_handle_changed.upgrade() {
            let mut conf = config_changed.lock().unwrap();
    ...
    let config_file_save = Arc::clone(&config_atom);
    ui.on_save_settings(move || {
        let conf = config_file_save.lock().unwrap();
    ...
    let config_tray = Arc::clone(&config_atom);

    timer.start(
        slint::TimerMode::Repeated,
        Duration::from_millis(30),
        move || match tray_manager.update() {
